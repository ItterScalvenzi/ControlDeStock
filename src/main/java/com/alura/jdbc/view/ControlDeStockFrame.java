...
public class ControlDeStockFrame extends JFrame {
	...
	private void configurarTablaDeContenido(Container container) {
		...
		botonEliminar.setBounds(10, 500, 110, 20);
		botonModificar.setBounds(130, 500, 110, 20);
		botonReporte.setBounds(250, 500, 110, 20);
	...
	private void modificar() {
		...
		// Codigo para verificar si el click esta dentro los limites del array (esto se da luego de que 
		//se hace click en boton modificar y luego se vuelve a hacer click fuera de la tabla y se aprieta
		//el boton modificar
		if (tabla.getSelectedRow() >= 0 && tabla.getSelectedRow() < modelo.getRowCount()
				&& tabla.getSelectedColumn() >= 0 && tabla.getSelectedColumn() < modelo.getColumnCount()) {
			Optional.ofNullable(modelo.getValueAt(tabla.getSelectedRow(), tabla.getSelectedColumn()))
					.ifPresentOrElse(fila -> {
						Integer id = Integer.valueOf(modelo.getValueAt(tabla.getSelectedRow(), 0).toString());
						String nombre = (String) modelo.getValueAt(tabla.getSelectedRow(), 1);
						String descripcion = (String) modelo.getValueAt(tabla.getSelectedRow(), 2);
						// Probamos con cantidad
						...
						JOptionPane.showMessageDialog(this, filasModificadas + " filas modificado con Éxito!");
